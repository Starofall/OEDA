...
class NonLocal: last_timestamp = '2017-12-04 16:59:46'

class StageResultsWithExperimentIdController(Resource):
    def get(self, experiment_id, stage_no):
        try:
            if NonLocal.last_timestamp:
                resp = jsonify(ExperimentResults=json.dumps(get_data_points_from(experiment_id, stage_no, NonLocal.last_timestamp)), sort_keys=True)
            else:
    ...
    def get(self, experiment_id):
        try:
            ...
            for stage in new_stages:
                if NonLocal.last_timestamp:
                    data = get_data_points_from(experiment_id, stage['number'], NonLocal.last_timestamp)
                else:
                ...
                #     print "data[8] " + str(data[8])
                #     print "data[9] " + str(data[9])
                #     print "data[len(data)-1] " + str(data[len(data)-1])
                # else:
                #     print "no data"
            ...
            # print "all_stage_data[0] " + str(all_stage_data[0])
            # print "all_stage_data[1] " + str(all_stage_data[1])
            # print "all_stage_data[2] " + str(all_stage_data[2])
            # print "all_stage_data[3] " + str(all_stage_data[3])
            # print "all_stage_data[4] " + str(all_stage_data[4])
            # print "all_stage_data[5] " + str(all_stage_data[5])
            # print "all_stage_data[6] " + str(all_stage_data[6])
            # print "all_stage_data[7] " + str(all_stage_data[7])
            # print "all_stage_data[8] " + str(all_stage_data[8])
            # print "all_stage_data[9] " + str(all_stage_data[9])

            print "all_stage_data " + str(len(all_stage_data))
            NonLocal.last_timestamp = all_stage_data[len(all_stage_data) - 1]["created"]
            # timestamp = sorted(all_stage_data, key=lambda point: point["created"])[0]
...
def get_data_points_from(experiment_id, stage_no, timestamp):
    # res = db().get_data_points(experiment_id=experiment_id, stage_no=stage_no)
